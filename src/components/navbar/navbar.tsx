...
import { IoGrid,  IoChatbubblesSharp, IoCreate, IoSearch, IoArrowDown, IoClose  } from "react-icons/io5";
...
const navbar = () => {
  ...
  return (
    ...
    <nav aria-label="Navigação Principal" className="flex flex-row w-full h-auto bg-rickblack font-semibold">
        <div className="flex flex-row w-full items-center py-2">
            ...
            <figure className="flex flex-row items-center gap-3  lg:w-1/2">
                <Link className="px-3" href="#" aria-label="Ir para a página inicial">
                    ...
                    src={Logo}
                    alt='Logo clickTips'
                    className='md:w-24 lg:w-28 h-auto'
                    />
                </Link>
             ...
             {/*Search and Navigation Section*/} 
                <div className="flex flex-row w-full ">
                    <form action="" className="flex items-center w-full">
                        ...
                        <input type="text" id="search-input" className="h-16 text-2xl rounded-l-lg border-none w-full px-3 text-rickblack" placeholder="Procure no ClickTips..."></input>
                        <button className="text-rickblack bg-white active:shadow-inner active:shadow-black w-auto h-16 flex items-center justify-center gap-2 border-2 hover:border-4 border-gray-200 hover:border-PrussianBlue px-4">
                            <HiMapPin className='text-3xl'/>
                            BA
                            <MdKeyboardArrowDown className='text-rickblack text-lg lg:text-3xl'/>
                        </button>
                        <button type="submit" className="bg-white active:shadow-inner active:shadow-black w-20 h-16 rounded-e-xl flex items-center justify-center focus:outline-none focus:ring focus:ring-PrussianBlue group">
                            <IoSearch className='text-rickblack text-5xl  rounded-full  p-2 group-hover:bg-PrussianBlue group-hover:text-white'/>
                        </button>
                    </form>

                    {/*Navigation Links */}
                    <div className="flex md:gap-3 lg:gap-6  w-1/3 justify-center items-center md:text-lg lg:text-2xl ">
                        <Button text = 'Login' action ='#' type='submit' aria_label='Ir até o Login'/>
                        <Button text ='Registrar' action ='#' type='submit' aria_label='ir até o registrar conta'/>
                    </div>
                </div>
        </div>
    </nav>
    
    <nav aria-label="Navigação Secundária" className="flex flex-row w-full h-auto justify-between text-center bg-PictonBlue text-xl">
        <ul className="flex list-none gap-10 lg:gap-16 px-8 lg:px-12 items-center lg:py-3">
...
<header className=" md:hidden" id="mobile-header" aria-labelledby="mobile-header-label" aria-hidden="true">
    <section className='bg-rickblack'>
